<div class="page-header">
    <div>
        <h4><i class="fas fa-headset me-2"></i>Telecaller Management</h4>
        <nav aria-label="breadcrumb"><ol class="breadcrumb"><li class="breadcrumb-item"><a href="<?= BASE_URL ?>/dashboard">Home</a></li><li class="breadcrumb-item active">Telecallers</li></ol></nav>
    </div>
    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#memberModal"><i class="fas fa-plus me-1"></i>Add Telecaller</button>
</div>
    ...
    <div class="card-body">
        <form method="GET" action="<?= BASE_URL ?>/members" class="row g-3">
            ...
            <div class="col-md-6 d-flex gap-1">
                <input type="text" name="search" class="form-control form-control-sm" placeholder="Search telecallers..." value="<?= e($_GET['search']??'') ?>">
    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                <thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Shift</th><th>Actions</th></tr></thead>
                <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <button class="action-btn btn-edit" title="Edit" onclick="openEditModal(<?= $m['id'] ?>,'<?= e($m['name']) ?>','<?= e($m['email']) ?>','<?= e($m['phone']) ?>','<?= e($m['shift']) ?>')"><i class="fas fa-edit"></i></button>
                ...
                <tr><td colspan="5"><div class="empty-state"><i class="fas fa-headset"></i><h6>No telecallers found</h6></div></td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
...
<div class="modal fade" id="memberModal" tabindex="-1">
    <div class="modal-dialog modal-lg"><div class="modal-content">
        <div class="modal-header bg-primary text-white">
            <h5 class="modal-title" id="memberModalTitle"><i class="fas fa-user-plus me-2"></i>Add Telecaller</h5>
        ...
        <form id="memberForm" method="POST" action="<?= BASE_URL ?>/members/store">
            <div class="modal-body row g-3">
                ...
                <input type="hidden" name="role" id="mRole" value="Telecaller">
                <input type="hidden" name="team_id" id="mTeam" value="">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i>Save Telecaller</button>
            </div>
        </form>
    </div></div>
</div>
...
<script>
function openEditModal(id, name, email, phone, shift) {
    document.getElementById('memberModalTitle').innerHTML = '<i class="fas fa-edit me-2"></i>Edit Telecaller';
...
function document_ready() {
    document.getElementById('memberModal').addEventListener('hidden.bs.modal', function () {
        document.getElementById('memberModalTitle').innerHTML = '<i class="fas fa-user-plus me-2"></i>Add Telecaller';
